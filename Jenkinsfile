node {
   ...
   stage('Preparation') {
       git 'https://github.com/kurama027/persona-service.git'
       mvnHome = tool 'MAVEN'
   }
   ...
   stage('Deployment') {
       try {
			sh 'chown jenkins:jenkins /var/lib/jenkins/workspace/helena-persona/runDeployment.sh'
			sh 'chmod +x /var/lib/jenkins/workspace/helena-persona/runDeployment.sh'
           sh '/var/lib/jenkins/workspace/helena-persona/runDeployment.sh'
       }catch (e){
